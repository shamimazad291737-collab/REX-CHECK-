...
def add_user(user_id, username):
    users_col.update_one(
        ...
        {
            "$setOnInsert": {
                "balance": Decimal128(Decimal("0")),
                "total_recharge": Decimal128(Decimal("0")),
...
def is_user_approved(user_id):
    if user_id == ADMIN_ID:
        return True
    user = users_col.find_one({"user_id": user_id}, {"approval_status": 1})
    return bool(user and user.get("approval_status") == "APPROVED")

def send_approval_request(user_id, first_name, username):
    existing = users_col.find_one({"user_id": user_id}, {"approval_status": 1})
    if existing and existing.get("approval_status") == "PENDING":
        # Only notify the admin once while the request is pending.
        pending_notified = existing.get("approval_request_sent", False)
        if pending_notified:
            return

    safe_name = html.escape(first_name or "User")
    safe_username = html.escape(username or "NoUsername")
    admin_markup = {
        "inline_keyboard": [
            [
                {"text": "✅ Approve", "callback_data": f"user_approve_{user_id}"},
                {"text": "❌ Reject", "callback_data": f"user_reject_{user_id}"}
            ]
        ]
    }
    admin_text = (
        ...
        "এই ইউজারকে বট ব্যবহার করার অনুমতি দিতে নিচের বাটন চাপুন।"
    )
    result = send_message(ADMIN_ID, admin_text, reply_markup=admin_markup)
    if result.get("ok"):
        users_col.update_one(
            {"user_id": user_id},
            {"$set": {"approval_request_sent": True, "approval_status": "PENDING"}},
            upsert=True
        )
...
def handle_update(update):
    try:
        ...
        if "message" in update:
            ...
            # Admin approval security guard for normal users
            if not is_admin and not is_user_approved(user_id):
                if text == "/start":
                    send_approval_request(user_id, first_name, username)
                send_message(chat_id, "⏳ <b>Wait — Admin approval request sent.</b>\nঅ্যাডমিন অনুমোদন করলে তারপর আপনি বট ব্যবহার করতে পারবেন।")
                return
            ...
            # Admin approval security guard for callback buttons
            if not is_admin and not is_user_approved(user_id):
                send_message(chat_id, "⏳ <b>Wait — Admin approval request sent.</b>\nঅ্যাডমিন অনুমোদন করলে তারপর আপনি বট ব্যবহার করতে পারবেন।")
                return
            ...
            # User approval callbacks (admin only)
            if data.startswith("user_approve_") and is_admin:
                target_user = int(data.replace("user_approve_", ""))
                users_col.update_one(
                    {"user_id": target_user},
                    {"$set": {"approval_status": "APPROVED", "approval_request_sent": True}}
                )
                edit_message(chat_id, message_id, "✅ <b>User approved successfully.</b>")
                send_message(target_user, "🎉 <b>Admin approved your request!</b>\nএখন আপনি বট ব্যবহার করতে পারবেন।", reply_markup=get_main_keyboard(False))
                return

            if data.startswith("user_reject_") and is_admin:
                target_user = int(data.replace("user_reject_", ""))
                users_col.update_one(
                    {"user_id": target_user},
                    {"$set": {"approval_status": "REJECTED", "approval_request_sent": True}}
                )
                edit_message(chat_id, message_id, "❌ <b>User request rejected.</b>")
                send_message(target_user, "❌ <b>Your admin approval request was rejected.</b>")
                return
