...
# Bot-specific settings namespace: allows multiple bots to share the same Mongo URI/database safely.
BOT_INSTANCE_KEY = hashlib.sha256((TOKEN or "").encode()).hexdigest()[:16]
BOT_STATUS_KEY = f"bot_status_{BOT_INSTANCE_KEY}"
...
def init_settings():
    ...
    if not settings_col.find_one({"key": BOT_STATUS_KEY}):
        settings_col.insert_one({"key": BOT_STATUS_KEY, "value": "ON"})
...
def get_bot_status():
    doc = settings_col.find_one({"key": BOT_STATUS_KEY})
    return doc["value"] if doc else "ON"

def set_bot_status(status):
    settings_col.update_one({"key": BOT_STATUS_KEY}, {"$set": {"value": status}}, upsert=True)
