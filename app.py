...
tg_stock_col = db[f"tg_stock_{TOKEN.replace(':', '_')}"] if TOKEN else db["tg_stock_default"]
...
def init_settings():
    ...
    if not settings_col.find_one({"key": "tg_price"}):
        settings_col.insert_one({"key": "tg_price", "value": "0.10"})
...
def get_bot_status():
    return "ON" if BOT_ACTIVE else "OFF"
...
def get_tg_price():
    doc = settings_col.find_one({"key": "tg_price"})
    return to_decimal(doc["value"]) if doc else Decimal("0.10")

def set_tg_price(new_price):
    price = to_decimal(new_price)
    if price < 0 or not price.is_finite():
        raise ValueError("Invalid price")
    settings_col.update_one(
...
def save_vpn_order(user_id, service, amount):
    ...
    res = vpn_orders_col.insert_one({
        "user_id": user_id,
        "service": service,
        "amount": Decimal128(to_decimal(amount)),
        "status": "PENDING",
        "date": now_str
    })
...
def add_user(user_id, username):
    users_col.update_one(
        {"user_id": user_id},
        {"$setOnInsert": {"balance": Decimal128(Decimal("0")), "total_recharge": Decimal128(Decimal("0")), "blocked": False}, "$set": {"username": username}},
        upsert=True
    )
...
def block_user(user_id):
    users_col.update_one({"user_id": user_id}, {"$set": {"blocked": True}}, upsert=True)

def get_banned_users():
    return list(users_col.find({"blocked": True}).sort("_id", -1))
...
def add_tg_stock_item(phone, link):
    tg_stock_col.insert_one({"phone_number": phone, "otp_link": link})

def pop_tg_stock_item():
    item = tg_stock_col.find_one_and_delete({})
    if item:
        return item["phone_number"], item["otp_link"]
    return None, None
...
def get_admin_panel_data():
    ...
    msg = (
        ...
        f"📲 <b>Telegram Number Price:</b> ${format_number_price(tg_pr)} USD\n"
    ...
    markup = {
        "inline_keyboard": [
            ...
            [{"text": "📲 Change Telegram Number Price", "callback_data": "admin_set_tg_rate", "style": "primary"}],
            ...
            [{"text": "📲 Upload Telegram Number Stock", "callback_data": "admin_upload_tg_file", "style": "success"}],
...
def render_buyer_page(target_u_id, page=1, items_per_page=10):
    ...
    nav_buttons.append([{"text": f"➖ Minus Balance from @{u_info[1]}", "callback_data": f"admin_minus_input_{target_u_id}"}])
...
def render_user_page(target_u_id, page=1, items_per_page=10):
    ...
    if total_pages > 1:
        row = []
        if page > 1:
            row.append({"text": f"◀️ Page {page-1}", "callback_data": f"pu_{target_u_id}_{page-1}"})
        row.append({"text": f"📄 {page}/{total_pages}", "callback_data": "noop"})
        if page < total_pages:
            row.append({"text": f"Page {page+1} ▶️", "callback_data": f"pu_{target_u_id}_{page+1}"})
        nav_buttons.append(row)

    nav_buttons.append([{"text": "🚫 REMOVE USER", "callback_data": f"admin_remove_user_{target_u_id}"}])
    nav_buttons.append([{"text": f"➕ Add / Refund Balance to @{u_info[1]}", "callback_data": f"admin_ref_input_{target_u_id}"}])
    nav_buttons.append([{"text": f"➖ Minus Balance from @{u_info[1]}", "callback_data": f"admin_minus_input_{target_u_id}"}])
    nav_buttons.append([{"text": "⬅️ Back to User List", "callback_data": "admin_view_users"}])
...
def handle_update(update):
    try:
        ...
        if "message" in update:
            ...
            if not is_admin and is_user_blocked(user_id):
                send_message(chat_id, "🚫 <b>আপনার বট অ্যাক্সেস সরিয়ে দেওয়া হয়েছে।</b>")
                return
            current_price = get_number_price()
            bdt_rate = get_bdt_per_usd()
            bot_active = (get_bot_status() == "ON")
            ...
            if is_admin and "reply_to_message" in msg:
                ...
                if "VPN ORDER #" in reply_text:
                    ...
                    if user_match:
                        ...
                        order_match = re.search(r'VPN ORDER #([a-f0-9]{24})', reply_text, re.I)
                        if order_match:
                            try:
                                vpn_orders_col.update_one(
                                    {"_id": ObjectId(order_match.group(1))},
                                    {"$set": {"status": "COMPLETED", "account_delivered": vpn_account_info, "completed_at": datetime.now()}}
                                )
                            except Exception as e:
                                print(f"VPN completion update error: {e}")

                        send_message(
                            target_u_id,
                            "🎉 <b>Your VPN order is completed!</b>\n\n"
                            f"🛡️ <b>VPN Details:</b>\n<code>{html.escape(vpn_account_info)}</code>\n\n"
                            "ধন্যবাদ আমাদের সার্ভিস ব্যবহার করার জন্য।"
                        )
            ...
            if state_data:
                ...
                if isinstance(state_data, str) and state_data == "BUY_TG_MULTI_QTY":
                    try:
                        qty = int(text.strip())
                        if qty <= 0:
                        ...
                        if qty > 20:
                            send_message(chat_id, "❌ <b>একসাথে সর্বোচ্চ ২০টি Telegram Number কিনতে পারবেন।</b> আবার BUY TG থেকে চেষ্টা করুন।")
                            delete_user_state(user_id)
                            return

                        u_info = get_user(user_id)
                        bal = u_info[2] if u_info else 0.0
                        total_cost = to_decimal(get_tg_price()) * Decimal(qty)

                        if to_decimal(bal) < to_decimal(total_cost):
                            send_message(chat_id, f"❌ <b>পর্যাপ্ত ব্যালেন্স নেই!</b>\n{qty} টি Telegram Number কিনতে ${format_number_price(total_cost)} USD লাগবে। আপনার ব্যালেন্স: ${format_money_exact(bal)} USD।")
                            delete_user_state(user_id)
                            return

                        tg_stock_count = tg_stock_col.count_documents({})
                        if tg_stock_count < qty:
                            send_message(chat_id, f"⚠️ <b>দুঃখিত! Telegram Number স্টকে পর্যাপ্ত নম্বর নেই।</b>\nবর্তমানে স্টকে {tg_stock_count} টি নম্বর খালি রয়েছে।")
                            delete_user_state(user_id)
                            return
                        ...
                        try:
                            for _ in range(qty):
                                phone, link = pop_tg_stock_item()
                                if not phone:
                                    raise RuntimeError("Telegram stock changed while processing the batch.")
                            ...
                            for p, l, oid in purchased_list:
                                add_tg_stock_item(p, l)
                                delete_active_order(oid)
                            delete_user_state(user_id)
                            send_message(chat_id, "⚠️ <b>Telegram Number order প্রস্তুত করতে সমস্যা হয়েছে।</b> আপনার ব্যালেন্স কাটা হয়নি। আবার চেষ্টা করুন।")
                            print(f"Telegram multi-buy preparation error: {e}")
                            return

                        res_msg = f"✅ <b>সফলভাবে {len(purchased_list)} টি Telegram Number কেনা হয়েছে!</b>\n\n"
                        for idx, (p, l, oid) in enumerate(purchased_list, 1):
                            res_msg += f"<b>{idx}.</b> 📱 <code>{html.escape(str(p))}</code> | 🔗 {html.escape(str(l))}\n"
                        res_msg += f"\n💰 <b>মোট ফি কাটা হবে:</b> ${format_number_price(total_cost)} USD\n👉 যেকোনো একটির ওটিপি পেতে <b>Check OTP</b> বাটনে চাপ দিন।"

                        multi_btns = []
                        for p, l, oid in purchased_list[:10]:
                            multi_btns.append([{"text": f"🔄 Check OTP ({p})", "callback_data": f"chk_otp_{p}"}])

                        delivery_result = send_message(chat_id, res_msg, reply_markup={"inline_keyboard": multi_btns})
                        if not delivery_result.get("ok"):
                            for p, l, oid in purchased_list:
                                add_tg_stock_item(p, l)
                                delete_active_order(oid)
                            delete_user_state(user_id)
                            send_message(chat_id, "⚠️ <b>Telegram Numberগুলো পাঠাতে সমস্যা হয়েছে।</b> আপনার ব্যালেন্স কাটা হয়নি। কিছুক্ষণ পর আবার চেষ্টা করুন।")
                            return
                        ...
                        if not deduct_balance(user_id, total_cost):
                            for p, l, oid in purchased_list:
                                ...
                                delete_active_order(oid)
                            delete_user_state(user_id)
                            send_message(chat_id, "⚠️ <b>পেমেন্ট প্রসেস করতে সমস্যা হয়েছে।</b> Telegram Numberগুলো স্টকে ফেরত দেওয়া হয়েছে এবং ব্যালেন্স কাটা হয়নি।")
                            return
                ...
                if is_admin and state_data == "ADMIN_SET_TG_PRICE":
                    try:
                        price_text = text.strip()
                        new_pr = to_decimal(price_text)
                        if new_pr < 0:
                            raise ValueError
                        set_tg_price(new_pr)
                        send_message(chat_id, f"✅ <b>Telegram নম্বরের নতুন মূল্য সেট করা হয়েছে: ${format_number_price(new_pr)} USD</b>", reply_markup=get_main_keyboard(is_admin))
                    except ValueError:
                        send_message(chat_id, "❌ <b>ভুল ইনপুট!</b> সঠিক সংখ্যা লিখুন (যেমন: 0.125)।")
                    delete_user_state(user_id)
                    return
                ...
                elif is_admin and state_data == "ADMIN_UPLOAD_TG_FILE":
                    if "document" in msg:
                        doc = msg["document"]
                        file_name = doc.get("file_name", "").lower()
                        if not (file_name.endswith(".txt") or file_name.endswith(".csv")):
                            ...
                            for line in content.splitlines():
                                ...
                                if len(parts) == 2:
                                    ...
                                    if phone.startswith("+") and (otp_link.startswith("http://") or otp_link.startswith("https://")):
                                        add_tg_stock_item(phone, otp_link)
                                        count += 1

                            send_message(chat_id, f"✅ <b>সফলভাবে {count} টি Telegram Number স্টকে আপলোড করা হয়েছে!</b>", reply_markup=get_main_keyboard(is_admin))
                            delete_user_state(user_id)
                            return
                    else:
                        send_message(chat_id, "❌ <b>অনুগ্রহ করে একটি সঠিক টেক্সট (.txt / .csv) ফাইল আপলোড করুন।</b>", reply_markup=get_back_keyboard())
                ...
                elif isinstance(state_data, str) and state_data.startswith("ADMIN_MINUS_USER_"):
                    target_user = int(state_data.replace("ADMIN_MINUS_USER_", ""))
                    try:
                        usd_val = to_decimal(text.strip())
                        if usd_val <= 0:
                            raise ValueError
                        ...
                        if not deduct_balance(target_user, usd_val):
                            current_info = get_user(target_user)
                            current_balance = current_info[2] if current_info else Decimal("0")
                            send_message(chat_id, f"❌ <b>ব্যালেন্স কমানো যায়নি।</b>\nবর্তমান ব্যালেন্স: ${format_money_exact(current_balance)} USD\nকমানোর পরিমাণ: ${format_money_exact(usd_val)} USD")
                        else:
                            new_info = get_user(target_user)
                            new_balance = new_info[2] if new_info else Decimal("0")
                            send_message(chat_id, f"✅ <b>User @{target_name} (ID: {target_user})-এর ব্যালেন্স থেকে ${format_money_exact(usd_val)} USD কমানো হয়েছে।</b>\n💰 নতুন ব্যালেন্স: ${format_money_exact(new_balance)} USD", reply_markup=get_main_keyboard(is_admin))
                            send_message(target_user, f"ℹ️ <b>এডমিন আপনার অ্যাকাউন্ট থেকে ${format_money_exact(usd_val)} USD কমিয়েছেন।</b>\n💰 নতুন ব্যালেন্স: ${format_money_exact(new_balance)} USD")
                    except Exception:
                        send_message(chat_id, "❌ <b>ভুল ইনপুট!</b> শুধু একটি positive USD amount লিখুন। (যেমন: 0.10 বা 1.25)")
                    delete_user_state(user_id)
                    return
            ...
            if text == "/start":
                ...
                price_text = format_number_price(current_price)
                stock_count = stock_col.count_documents({})
                markup = {
                    "inline_keyboard": [
                        [{"text": "👤 BUY SINGLE 🇺🇲", "callback_data": "confirm_buy_usa"}],
                        [{"text": "🔢 BUY MULTIPLE 🇺🇲", "callback_data": "buy_multi_num"}]
                    ]
                }
                send_message(
                    chat_id,
                    "🛒 <b>BUY NUMBER</b> 🇺🇲\n\n"
                    f"💵 <b>Price:</b> ${price_text} USD / Number\n"
                    f"📦 <b>Available Stock:</b> {stock_count}\n\n"
                ...
                tg_pr = get_tg_price()
                price_text = format_number_price(tg_pr)
                tg_stock_count = tg_stock_col.count_documents({})
                markup = {
                    "inline_keyboard": [
                        [{"text": "👤 BUY SINGLE TG", "callback_data": "confirm_buy_tg"}],
                        [{"text": "🔢 BUY MULTIPLE TG", "callback_data": "buy_multi_tg"}]
                    ]
                }
                send_message(
                    chat_id,
                    "📲 <b>BUY TELEGRAM NUMBER</b>\n\n"
                    f"💵 <b>Price:</b> ${price_text} USD / Number\n"
                    f"📦 <b>Available Stock:</b> {tg_stock_count}\n\n"
                    "👇 আপনার প্রয়োজন অনুযায়ী একটি অপশন নির্বাচন করুন:",
                    reply_markup=markup
                )
                ...
                nord_pr = get_vpn_price("nord")
                proton_pr = get_vpn_price("proton")
                nord_stock = get_vpn_stock_count("nord")
                proton_stock = get_vpn_stock_count("proton")
                markup = {
                    "inline_keyboard": [
                        [{"text": f"🛡️ NordVPN  •  ${nord_pr:.2f}  •  Stock {nord_stock}", "callback_data": "buy_vpn_nord"}],
                        [{"text": f"🛡️ ProtonVPN  •  ${proton_pr:.2f}  •  Stock {proton_stock}", "callback_data": "buy_vpn_proton"}]
                    ]
                }
                send_message(
                    chat_id,
            ...
            if data.startswith("buy_vpn_"):
                ...
                stock_count = get_vpn_stock_count(service)
                u_info = get_user(user_id)
                bal = to_decimal(u_info[2] if u_info else 0)

                if stock_count <= 0:
                    edit_message(
                        chat_id, message_id,
                        f"⚠️ <b>{service.capitalize()}VPN বর্তমানে Out of Stock!</b>\n\n"
                        "স্টক আসার পর আবার চেষ্টা করুন।"
                    )
                    return
                ...
                if bal < pr:
                    edit_message(
                        chat_id, message_id,
                        f"❌ <b>পর্যাপ্ত ব্যালেন্স নেই!</b>\n\n"
                        f"💵 মূল্য: ${format_money_exact(pr)} USD\n"
                        f"💰 আপনার ব্যালেন্স: ${format_money_exact(bal)} USD\n\n"
                        "প্রথমে Deposit করে আবার চেষ্টা করুন।"
                    )
                    return
                ...
                conf_markup = {
                    "inline_keyboard": [
                        [
                            {"text": "✅ CONFIRM ORDER", "callback_data": f"conf_vpn_{service}"},
                            {"text": "❌ CANCEL", "callback_data": "cancel_vpn"}
                        ]
                    ]
                }
                edit_message(
                    chat_id, message_id,
                    f"🛡️ <b>{service.upper()}VPN ORDER</b>\n\n"
                    f"💵 Price: <b>${format_money_exact(pr)} USD</b>\n"
                    f"📦 Stock: <b>{stock_count}</b>\n"
                    f"💰 Your Balance: <b>${format_money_exact(bal)} USD</b>\n\n"
                    "অর্ডারটি কনফার্ম করতে নিচের Confirm বাটনে চাপুন।",
                    reply_markup=conf_markup
                )
                return

            elif data.startswith("conf_vpn_"):
                service = "nord" if data == "conf_vpn_nord" else "proton"
                pr = to_decimal(get_vpn_price(service))
                u_info = get_user(user_id)
                bal = to_decimal(u_info[2] if u_info else 0)

                if get_vpn_stock_count(service) <= 0:
                ...
                if not deduct_balance(user_id, pr):
                    edit_message(
                        chat_id, message_id,
                        f"❌ <b>অর্ডার সম্পন্ন করা যায়নি।</b>\nআপনার বর্তমান ব্যালেন্স: ${format_money_exact(bal)} USD।"
                    )
                    return
                ...
                try:
                    ...
                    vpn_orders_col.update_one(
                        ...
                        {"$set": {"account": account, "status": "PENDING", "updated_at": datetime.now()}}
                    )
                except Exception as e:
                    add_vpn_stock_item(service, account)
                    add_refund_balance(user_id, pr)
                    edit_message(chat_id, message_id, "⚠️ <b>অর্ডার তৈরি করতে সমস্যা হয়েছে।</b> আপনার টাকা ফেরত দেওয়া হয়েছে।")
                    print(f"VPN order creation error: {e}")
                    return
                ...
                admin_notif = (
                    ...
                    f"💰 <b>Price:</b> ${format_money_exact(pr)} USD\n\n"
                    "👉 এই মেসেজে <b>Reply</b> করে ইউজারের VPN account details পাঠান।"
                )
                ...
                if not admin_result.get("ok"):
                    ...
                    add_vpn_stock_item(service, account)
                    add_refund_balance(user_id, pr)
                    edit_message(chat_id, message_id, "⚠️ <b>অর্ডার নোটিফিকেশন পাঠানো যায়নি।</b> আপনার টাকা ফেরত দেওয়া হয়েছে।")
                    return
            ...
            if data == "admin_toggle_bot_off" and is_admin:
            ...
            elif data == "buy_multi_tg":
                set_user_state(user_id, "BUY_TG_MULTI_QTY")
                send_message(chat_id, "🔢 <b>আপনি কতগুলো Telegram Number কিনতে চান লিখে পাঠান:</b>\n(যেমন: 2, 5, 10 ইত্যাদি)", reply_markup=get_back_keyboard())

            elif data == "confirm_buy_tg":
                u_info = get_user(user_id)
                bal = u_info[2] if u_info else 0.0

                if to_decimal(bal) < to_decimal(get_tg_price()):
                    edit_message(chat_id, message_id, f"❌ <b>পর্যাপ্ত ব্যালেন্স নেই!</b>\nTelegram Number কিনতে অন্তত ${format_number_price(get_tg_price())} USD ব্যালেন্স লাগবে। Deposit সেকশন থেকে রিচার্জ করুন।")
                    return

                phone, link = pop_tg_stock_item()
                if not phone:
                    edit_message(chat_id, message_id, "⚠️ <b>দুঃখিত! বর্তমানে Telegram Number স্টক ফাঁকা রয়েছে।</b> কিছু সময় পর আবার চেষ্টা করুন।")
                    return

                markup = {
                    "inline_keyboard": [
                        [{"text": "🔄 Check OTP", "callback_data": f"chk_otp_{phone}"}],
                        [{"text": "📲 Buy Another TG Number", "callback_data": "confirm_buy_tg"}]
                    ]
                }
                res_text = (
                    f"✅ <b>Telegram Number বরাদ্দ করা হয়েছে!</b>\n\n"
                    f"📱 <b>Telegram Number:</b> <code>{html.escape(str(phone))}</code>\n"
                    f"🔗 <b>OTP Link:</b> {link}\n"
                    f"💰 <b>ফি কাটা হয়েছে:</b> ${format_number_price(get_tg_price())} USD\n\n"
                ...
                try:
                    ...
                    add_tg_stock_item(phone, link)
                    edit_message(chat_id, message_id, "⚠️ <b>Telegram Number order তৈরি করতে সমস্যা হয়েছে।</b> আপনার ব্যালেন্স কাটা হয়নি। আবার চেষ্টা করুন।")
                    print(f"Telegram single-buy order creation error: {e}")
                    return
                ...
                if not delivery_result.get("ok"):
                    ...
                    edit_message(chat_id, message_id, "⚠️ <b>Telegram Number পাঠাতে সমস্যা হয়েছে।</b> আপনার ব্যালেন্স কাটা হয়নি। কিছুক্ষণ পর আবার চেষ্টা করুন।")
                    return

                if not deduct_balance(user_id, current_price):
                    add_tg_stock_item(phone, link)
                    delete_active_order(order_id)
                    edit_message(chat_id, message_id, "⚠️ <b>পেমেন্ট প্রসেস করতে সমস্যা হয়েছে।</b> Telegram Numberটি আবার স্টকে ফেরত দেওয়া হয়েছে এবং ব্যালেন্স কাটা হয়নি।")
                    return
                ...
                try:
                    ...
                    if re.match(r"^\d{3,10}$", otp_text):
                        message = f"<blockquote><b>Your WhatsApp OTP:</b> <pre>{otp_text}</pre></blockquote>"
                        otp_markup = {"inline_keyboard": [[{"text": "📋 Copy OTP", "copy_text": {"text": otp_text}, "style": "success"}]]}
            
                        group_chat_id = os.getenv("OTP_GROUP_ID")
                        if group_chat_id:
                            group_message = f"<blockquote>🔔 <b>New WhatsApp OTP Received!</b>\n📱 Number: <code>{phone}</code>\n🔑 OTP: <pre>{otp_text}</pre></blockquote>"
                            group_markup = {"inline_keyboard": [[{"text": "📋 Copy OTP", "copy_text": {"text": otp_text}, "style": "success"}]]}
                            send_message(int(group_chat_id), group_message, reply_markup=group_markup)
                    else:
                        message = "⏳ <b>এখনো ওটিপি আসেনি বা কোড পাওয়া যায়নি। একটু পরে আবার চেক করুন।</b>"
            
                    send_message(chat_id, message, reply_markup=otp_markup if re.match(r"^\d{3,10}$", otp_text) else None)
                except Exception as e:
                ...
                markup = {
                    "inline_keyboard": [
                        ...
                        [{"text": "🚫 Ban User History", "callback_data": "admin_ban_history"}],
                        [{"text": "🛒 Active Buyers List", "callback_data": "admin_view_buyers"}],
                        [{"text": "📜 Refund History Log", "callback_data": "admin_refund_history"}],
                ...
                for u in users_list:
                    ...
                    buttons.append([{"text": display_title, "callback_data": f"pu_{u_id}_1"}])

                buttons.append([{"text": "⬅️ Back to User Management", "callback_data": "admin_user_mgmt_menu"}])
            ...
            elif data == "admin_ban_history" and is_admin:
                banned_users = get_banned_users()
                if not banned_users:
                    edit_message(chat_id, message_id, "🚫 <b>কোনো banned user নেই।</b>", reply_markup={"inline_keyboard": [[{"text": "⬅️ Back to User Management", "callback_data": "admin_user_mgmt_menu"}]]})
                    return

                buttons = []
                for banned in banned_users:
                    b_id = banned.get("user_id")
                    b_name = banned.get("username", "NoUsername")
                    display_title = f"🚫 @{b_name} (ID: {b_id})" if b_name != "NoUsername" else f"🚫 ID: {b_id}"
                    buttons.append([
                        {"text": display_title, "callback_data": f"admin_unban_user_{b_id}"}
                    ])
                buttons.append([{"text": "⬅️ Back to User Management", "callback_data": "admin_user_mgmt_menu"}])
                edit_message(chat_id, message_id, f"🚫 <b>BAN USER HISTORY</b>\n\nমোট banned user: {len(banned_users)} জন\nUnban করতে user-এর button-এ চাপুন:", reply_markup={"inline_keyboard": buttons})

            elif data.startswith("admin_unban_user_") and is_admin:
                target_u_id = int(data.replace("admin_unban_user_", ""))
                unblock_user(target_u_id)
                edit_message(chat_id, message_id, "✅ <b>User successfully unbanned.</b>", reply_markup={"inline_keyboard": [[{"text": "🚫 Ban User History", "callback_data": "admin_ban_history"}], [{"text": "⬅️ Back to User Management", "callback_data": "admin_user_mgmt_menu"}]]})
                ...
                for u in buyers_list:
                    u_id, u_name, u_bal = u[0], u[1], u[2]
                    display_title = f"🛒 @{u_name} (${format_money_exact(u_bal)})" if u_name != "NoUsername" else f"🛒 ID: {u_id} (${format_money_exact(u_bal)})"
                    buttons.append([{"text": display_title, "callback_data": f"pb_{u_id}_1"}])
                ...
                markup = {"inline_keyboard": [[{"text": "⬅️ Back to User Management", "callback_data": "admin_user_mgmt_menu"}]]}
                edit_message(chat_id, message_id, history_msg, reply_markup=markup)

            elif data.startswith("admin_remove_user_") and is_admin:
                target_u_id = int(data.replace("admin_remove_user_", ""))
                if target_u_id == ADMIN_ID:
                    edit_message(chat_id, message_id, "❌ <b>Admin user-কে remove করা যাবে না।</b>")
                else:
                    block_user(target_u_id)
                    edit_message(chat_id, message_id, "🚫 <b>User successfully removed.</b>\n\nএই user আবার /start দিলেও access পাবে না, যতক্ষণ না database থেকে unblock করা হয়.")
                ...
                markup = {
                    "inline_keyboard": [
                        [{"text": "🛡️ Upload NordVPN", "callback_data": "up_vpn_nord"}],
                        [{"text": "🛡️ Upload ProtonVPN", "callback_data": "up_vpn_proton"}]
                    ]
                }
            ...
            elif data.startswith("admin_minus_input_") and is_admin:
                target_u_id = int(data.replace("admin_minus_input_", ""))
                u = get_user(target_u_id)
                target_uname = u[1] if u else "User"
                set_user_state(user_id, f"ADMIN_MINUS_USER_{target_u_id}")
                current_balance = u[2] if u else Decimal("0")
                send_message(chat_id, f"➖ <b>User @{target_uname} (ID: {target_u_id})-এর ব্যালেন্স থেকে কত $ (USD) কমাতে চান লিখে পাঠান:</b>\n\n💰 বর্তমান ব্যালেন্স: <b>${format_money_exact(current_balance)} USD</b>\n<i>শুধু positive amount দিন, যেমন: <code>0.125</code> বা <code>1.50</code></i>", reply_markup=get_back_keyboard())
            ...
            elif data == "admin_set_tg_rate" and is_admin:
                set_user_state(user_id, "ADMIN_SET_TG_PRICE")
                tg_pr = get_tg_price()
                send_message(chat_id, f"📲 <b>Telegram নম্বরের নতুন মূল্য ($ USD) লিখে পাঠান:</b>\n(বর্তমান রেট: ${format_number_price(tg_pr)} USD)", reply_markup=get_back_keyboard())
            ...
            elif data == "admin_upload_tg_file" and is_admin:
                set_user_state(user_id, "ADMIN_UPLOAD_TG_FILE")
                send_message(chat_id, "📲 <b>Telegram Number সম্বলিত ফাইলটি (.txt / .csv) এখানে পাঠাও:</b>\nফরম্যাট:\n<code>+1234567890, https://otp-link.com/check</code>", reply_markup=get_back_keyboard())
                ...
                stock_items = get_all_stock()
                tg_count = tg_stock_col.count_documents({})
                ...
                stock_text = (
                    ...
                    f"📲 <b>Telegram Number Stock:</b> {tg_count} টি\n"
                ...
                markup = {
                    "inline_keyboard": [
                        [{"text": "🛡️ Delete All VPN Stock", "callback_data": "admin_delete_vpn_stock_execute"}],
                        [{"text": "📱 Delete All Number Stock", "callback_data": "admin_delete_number_stock_execute"}]
                    ]
                }
                edit_message(chat_id, message_id, "🗑️ <b>কোন স্টক ডিলিট করতে চান?</b>", reply_markup=markup)

            elif data == "admin_delete_vpn_stock_execute" and is_admin:
                vpn_stock_col.delete_many({})
                edit_message(chat_id, message_id, "🗑️ <b>সফলভাবে সমস্ত VPN স্টক ডিলিট করা হয়েছে!</b>")

            elif data == "admin_delete_number_stock_execute" and is_admin:
                clear_all_stock()
                edit_message(chat_id, message_id, "🗑️ <b>সফলভাবে সমস্ত Number Stock ডিলিট করা হয়েছে!</b>")
