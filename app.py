...
def is_user_approved(user_id):
    """Existing users remain approved unless explicitly removed."""
    user = users_col.find_one({"user_id": user_id}, {"access_approved": 1})
    return user is not None and user.get("access_approved", True) is not False

def set_user_approval(user_id, approved):
    users_col.update_one(
        {"user_id": user_id},
        {"$set": {"access_approved": bool(approved)}},
        upsert=False
    )

def get_user_access_list(approved):
    users = list(users_col.find(
        {"access_approved": {"$ne": False}} if approved else {"access_approved": False},
...
def handle_update(update):
    try:
        ...
        if "message" in update:
            ...
            # Access control: removed users must be approved by admin before using the bot again.
            if not is_admin and not is_user_approved(user_id):
                send_message(
                    chat_id,
                    "🚫 <b>আপনার বট অ্যাক্সেস বন্ধ করা হয়েছে।</b>\n"
                    "আবার ব্যবহার করতে Admin-এর approval প্রয়োজন।"
                )
                return
            ...
            if data == "admin_toggle_bot_off" and is_admin:
                ...
                markup = {
                    "inline_keyboard": [
                        ...
                        [{"text": "🚫 Remove User Access", "callback_data": "admin_remove_user", "style": "danger"}],
                        [{"text": "✅ Approve User Access", "callback_data": "admin_approve_user", "style": "success"}],
            ...
            elif data == "admin_remove_user" and is_admin:
                users_list = get_user_access_list(True)
                if not users_list:
                    edit_message(
                        chat_id, message_id,
                        "❌ <b>Remove করার মতো কোনো approved user নেই।</b>",
                        reply_markup={"inline_keyboard": [[{"text": "⬅️ Back", "callback_data": "admin_user_mgmt_menu"}]]}
                    )
                    return
                buttons = []
                for u_id, u_name, u_bal in users_list:
                    title = f"🚫 @{u_name}" if u_name != "NoUsername" else f"🚫 ID: {u_id}"
                    buttons.append([{"text": title, "callback_data": f"admin_rm_{u_id}", "style": "danger"}])
                buttons.append([{"text": "⬅️ Back to User Management", "callback_data": "admin_user_mgmt_menu", "style": "danger"}])
                edit_message(
                    chat_id, message_id,
                    "🚫 <b>REMOVE USER ACCESS</b>\nযে user-এর access বন্ধ করতে চান তাকে নির্বাচন করুন:",
                    reply_markup={"inline_keyboard": buttons}
                )

            elif data == "admin_approve_user" and is_admin:
                users_list = get_user_access_list(False)
                if not users_list:
                    edit_message(
                        chat_id, message_id,
                        "ℹ️ <b>কোনো removed user নেই।</b>",
                        reply_markup={"inline_keyboard": [[{"text": "⬅️ Back", "callback_data": "admin_user_mgmt_menu"}]]}
                    )
                    return
                buttons = []
                for u_id, u_name, u_bal in users_list:
                    title = f"✅ @{u_name}" if u_name != "NoUsername" else f"✅ ID: {u_id}"
                    buttons.append([{"text": title, "callback_data": f"admin_ap_{u_id}", "style": "success"}])
                buttons.append([{"text": "⬅️ Back to User Management", "callback_data": "admin_user_mgmt_menu", "style": "danger"}])
                edit_message(
                    chat_id, message_id,
                    "✅ <b>APPROVE USER ACCESS</b>\nযে user-এর access চালু করতে চান তাকে নির্বাচন করুন:",
                    reply_markup={"inline_keyboard": buttons}
                )

            elif data.startswith("admin_rm_") and is_admin:
                try:
                    target_u_id = int(data.split("_")[-1])
                except ValueError:
                    return
                if target_u_id == ADMIN_ID:
                    edit_message(chat_id, message_id, "❌ <b>Admin-কে remove করা যাবে না।</b>")
                    return
                set_user_approval(target_u_id, False)
                edit_message(
                    chat_id, message_id,
                    f"🚫 <b>User {target_u_id} এর access বন্ধ করা হয়েছে।</b>\n"
                    "সে আবার ব্যবহার করতে চাইলে Admin approval প্রয়োজন হবে।",
                    reply_markup={"inline_keyboard": [[{"text": "⬅️ Back to User Management", "callback_data": "admin_user_mgmt_menu"}]]}
                )

            elif data.startswith("admin_ap_") and is_admin:
                try:
                    target_u_id = int(data.split("_")[-1])
                except ValueError:
                    return
                set_user_approval(target_u_id, True)
                edit_message(
                    chat_id, message_id,
                    f"✅ <b>User {target_u_id} approved.</b>\nএখন সে আবার bot ব্যবহার করতে পারবে।",
                    reply_markup={"inline_keyboard": [[{"text": "⬅️ Back to User Management", "callback_data": "admin_user_mgmt_menu"}]]}
                )
