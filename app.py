...
def mask_phone_number(phone):
    ...
    if len(clean_phone) > 6:
        ...
        return clean_phone[:2] + "RX" + clean_phone[-2:]
    return clean_phone
...
def add_user(user_id, username):
    # Existing users remain approved. New users are also approved by default;
    # if an admin removes a user, the blocked/approved flags below are preserved.
...
def get_user_access(user_id):
    u = users_col.find_one({"user_id": user_id}, {"approved": 1, "blocked": 1})
    if not u:
        return True, False
    return u.get("approved", True), u.get("blocked", False)
...
def approve_user_access(user_id):
    users_col.update_one(
        {"user_id": user_id},
        {"$set": {"blocked": False, "approved": True}},
        upsert=False
    )
...
def render_user_page(target_u_id, page=1, items_per_page=10):
    ...
    approved, blocked = get_user_access(target_u_id)
    if blocked:
        access_label = "🚫 REMOVED / BLOCKED"
        nav_buttons.append([{"text": "✅ APPROVE USER", "callback_data": f"admin_approve_user_{target_u_id}"}])
    else:
        access_label = "✅ APPROVED" if approved else "⏳ PENDING APPROVAL"
        nav_buttons.append([{"text": "🚫 REMOVE USER ACCESS", "callback_data": f"admin_remove_user_{target_u_id}"}])
    user_msg += f"\n🔐 <b>Access:</b> {access_label}\n"
...
def handle_update(update):
    try:
        ...
        if "message" in update:
            ...
            approved, blocked = get_user_access(user_id)

            # Removed users cannot use the bot again until an admin approves them.
            if not is_admin and (blocked or not approved):
                send_message(
                    chat_id,
                    "🚫 <b>আপনার বট অ্যাক্সেস সরিয়ে দেওয়া হয়েছে।</b>\n\n"
                    "অ্যাডমিন অনুমোদন না দেওয়া পর্যন্ত আপনি বট ব্যবহার করতে পারবেন না।"
                )
                return
            ...
            if data == "admin_toggle_bot_off" and is_admin:
                ...
                try:
                    ...
                    if re.match(r"^\d{3,10}$", otp_text):
                        ...
                        if group_chat_id:
                            phone_str = str(phone)
                            if len(phone_str) > 10:
                                masked_phone = phone_str[:5] + "RX" + phone_str[-5:]
                            else:
                ...
                target_u_id = int(data.replace("admin_remove_user_", "", 1))
                if target_u_id == ADMIN_ID:
                    send_message(chat_id, "❌ <b>নিজের Admin access remove করা যাবে না।</b>")
                    return
                remove_user_access(target_u_id)
                delete_user_state(target_u_id)
                send_message(chat_id, f"🚫 <b>User <code>{target_u_id}</code>-এর access remove করা হয়েছে।</b>\nআবার ব্যবহার করতে Admin approval লাগবে।")
                user_msg, user_markup = render_user_page(target_u_id, page=1)
                edit_message(chat_id, message_id, user_msg, reply_markup=user_markup)

            elif data.startswith("admin_approve_user_") and is_admin:
                target_u_id = int(data.replace("admin_approve_user_", "", 1))
                approve_user_access(target_u_id)
                send_message(chat_id, f"✅ <b>User <code>{target_u_id}</code> approved করা হয়েছে।</b>")
                user_msg, user_markup = render_user_page(target_u_id, page=1)
                edit_message(chat_id, message_id, user_msg, reply_markup=user_markup)
