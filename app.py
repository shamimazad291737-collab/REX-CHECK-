...
def render_buyer_page(target_u_id, page=1, items_per_page=10):
    ...
    nav_buttons.append([{"text": f"➖ Minus User Balance from @{u_info[1]}", "callback_data": f"admin_minus_input_{target_u_id}"}])
        ...
        if "message" in update:
            ...
            if state_data:
                ...
                if isinstance(state_data, dict) and state_data.get("step") == "WAITING_AMOUNT":
                    ...
                    try:
                        usd_val = to_decimal(text.strip())
                        if usd_val <= 0 or not usd_val.is_finite():
                            raise ValueError
                        ...
                        if not deduct_balance(target_user, usd_val):
                            ...
                            send_message(chat_id, f"❌ <b>ব্যালেন্স মাইনাস করা যায়নি।</b> User @{target_name}-এর বর্তমান ব্যালেন্স ${format_money_exact(current_balance)} USD, যা ${format_money_exact(usd_val)} USD-এর চেয়ে কম।")
                            delete_user_state(user_id)
                            return
                        ...
                        send_message(chat_id, f"✅ <b>User @{target_name} (ID: {target_user})-এর ব্যালেন্স থেকে ${format_money_exact(usd_val)} USD মাইনাস করা হয়েছে।</b>\n💰 <b>নতুন ব্যালেন্স:</b> ${format_money_exact(new_balance)} USD", reply_markup=get_main_keyboard(is_admin))
                        send_message(target_user, f"⚠️ <b>আপনার অ্যাকাউন্ট ব্যালেন্স থেকে ${format_money_exact(usd_val)} USD মাইনাস করা হয়েছে।</b>\n💰 <b>বর্তমান ব্যালেন্স:</b> ${format_money_exact(new_balance)} USD")
                    except Exception:
                        send_message(chat_id, "❌ <b>ভুল ইনপুট!</b> সঠিক USD অ্যামাউন্ট লিখুন। (যেমন: 0.50 বা 5)")
                    delete_user_state(user_id)
                    return
            ...
            if data == "admin_toggle_bot_off" and is_admin:
                ...
                send_message(chat_id, f"➖ <b>User @{target_uname} (ID: {target_u_id})-এর ব্যালেন্স থেকে কত $ (USD) মাইনাস করতে চান লিখে পাঠান:</b>\n\n<i>উদাহরণ: <code>5</code> বা <code>0.50</code></i>", reply_markup=get_back_keyboard())
