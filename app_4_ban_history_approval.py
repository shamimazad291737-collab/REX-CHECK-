...
def add_user(user_id, username):
    users_col.update_one(
        {"user_id": user_id},
        {"$setOnInsert": {"balance": Decimal128(Decimal("0")), "total_recharge": Decimal128(Decimal("0")), "blocked": False, "approved": False, "approval_requested": False}, "$set": {"username": username}},
        upsert=True
    )
...
def is_user_approved(user_id):
    user = users_col.find_one({"user_id": user_id}, {"approved": 1})
    # Existing users without the new field keep their current access.
    return True if user and "approved" not in user else bool(user and user.get("approved", False))

def approve_user(user_id):
    users_col.update_one({"user_id": user_id}, {"$set": {"approved": True, "approval_requested": False}})

def request_user_approval(user_id):
    result = users_col.update_one(
        {"user_id": user_id, "approval_requested": {"$ne": True}},
        {"$set": {"approved": False, "approval_requested": True}}
    )
    return result.modified_count == 1

def get_pending_approval_users():
    return list(users_col.find({"approved": False, "approval_requested": True}).sort("_id", -1))
...
def get_admin_panel_data():
    ...
    markup = {
        "inline_keyboard": [
            ...
            [{"text": "🛡️ Set NordVPN Price", "callback_data": "admin_set_nord_price"}, {"text": "🛡️ Set ProtonVPN Price", "callback_data": "admin_set_proton_price"}],
            [{"text": "📁 Upload VPN Stock File", "callback_data": "admin_upload_vpn_file"}],
    ...
    try:
        ...
        if "message" in update:
            ...
            if not is_admin and not is_user_approved(user_id):
                if text == "/start":
                    if request_user_approval(user_id):
                        send_message(chat_id, "⏳ <b>আপনার access approval request Admin-এর কাছে পাঠানো হয়েছে।</b>\nAdmin approve না করা পর্যন্ত বট ব্যবহার করতে পারবেন না।")
                        user = users_col.find_one({"user_id": user_id}, {"username": 1}) or {}
                        uname = user.get("username", "NoUsername")
                        admin_markup = {"inline_keyboard": [[{"text": "✅ APPROVE USER", "callback_data": f"admin_approve_user_{user_id}"}]]}
                        send_message(ADMIN_ID, f"🔔 <b>New User Approval Request</b>\n\n👤 Username: @{html.escape(uname)}\n🆔 User ID: <code>{user_id}</code>\n\nএই user-কে approve করতে নিচের button চাপুন:", reply_markup=admin_markup)
                    else:
                        send_message(chat_id, "⏳ <b>আপনার approval request এখনো pending আছে।</b>\nAdmin approve না করা পর্যন্ত বট ব্যবহার করতে পারবেন না।")
                else:
                    send_message(chat_id, "⏳ <b>আপনার access এখনো Admin approve করেননি।</b>\nআগে /start চাপুন এবং approval-এর জন্য অপেক্ষা করুন।")
                return
            ...
            if data == "admin_toggle_bot_off" and is_admin:
                ...
                markup = {
                    "inline_keyboard": [
                        ...
                        [{"text": "⏳ Pending Approval Requests", "callback_data": "admin_pending_approvals"}],
            ...
            elif data == "admin_pending_approvals" and is_admin:
                pending_users = get_pending_approval_users()
                if not pending_users:
                    edit_message(chat_id, message_id, "⏳ <b>কোনো pending approval request নেই।</b>", reply_markup={"inline_keyboard": [[{"text": "⬅️ Back to User Management", "callback_data": "admin_user_mgmt_menu"}]]})
                    return
                buttons = []
                for pending in pending_users:
                    p_id = pending.get("user_id")
                    p_name = pending.get("username", "NoUsername")
                    display_title = f"⏳ @{p_name} (ID: {p_id})" if p_name != "NoUsername" else f"⏳ ID: {p_id}"
                    buttons.append([{"text": display_title, "callback_data": f"admin_approve_user_{p_id}"}])
                buttons.append([{"text": "⬅️ Back to User Management", "callback_data": "admin_user_mgmt_menu"}])
                edit_message(chat_id, message_id, f"⏳ <b>PENDING APPROVAL REQUESTS</b>\n\nমোট pending user: {len(pending_users)} জন\nApprove করতে user-এর button-এ চাপুন:", reply_markup={"inline_keyboard": buttons})

            elif data.startswith("admin_approve_user_") and is_admin:
                target_u_id = int(data.replace("admin_approve_user_", ""))
                if target_u_id == ADMIN_ID:
                    edit_message(chat_id, message_id, "❌ <b>Admin user-এর approval প্রয়োজন নেই।</b>")
                else:
                    approve_user(target_u_id)
                    send_message(target_u_id, "✅ <b>Admin আপনার access approve করেছেন!</b>\nএখন আপনি বট ব্যবহার করতে পারবেন। /start চাপুন।")
                    edit_message(chat_id, message_id, "✅ <b>User successfully approved.</b>", reply_markup={"inline_keyboard": [[{"text": "⏳ Pending Approval Requests", "callback_data": "admin_pending_approvals"}], [{"text": "⬅️ Back to User Management", "callback_data": "admin_user_mgmt_menu"}]]})
